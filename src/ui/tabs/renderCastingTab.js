...
import { getShardMitosisPerSecond, getShardPerSecond } from "../../core/helpers/castingUpgradeHelpers.js";
...
function renderRecastSummaryInto(host, state) {
  ...
  if(canCast(state)) {
    ...
    if (hasUpgrade(state, "PRES00103", "castingUpgrades")) {
      ...
      createElement("div", {
        className: "muted",
        text: `Best Shards Per Cast: ${formatNumber(state.stats.bestShardsPerCast)}`
      }),
      createElement("div", {
        className: "muted",
        text: `Best Shards Per Second: ${formatNumber(state.stats.bestShardsPerCastPerSecond)}`
      }),
      createElement("div", {
        className: "muted",
        text: `Current Shards Per Second: ${formatNumber(getShardPerSecond(state))}`
      }));
    }
  } else {
    host.append(
