...
import { getCastingRewards } from "./castingHelpers.js";
...
export function getCastingUpgradeConfig(state) {
  ...
  const castMulti = getUpgradeLevel(state, "PRES10203", "castingUpgrades");
  if (castMulti > 0) {
    castGain = multiplyBigNum(castGain, powerBigNum(2, castMulti));
  }
...
export function getShardMitosisPerSecond(state) {
  ...
  return divideBigNumByNumber(state.stats.bestShardsPerCastPerSecond ?? zeroBigNum(), 5);
}
