...
export function renderUpgradesTab(state, setState) {
  ...
  const title = createElement("h2", { className: "panel-title", text: "Upgrade Tree" });

  const resetViewButton = createElement("button", {
    text: "Reset View",
    onClick: () => {
      setState((draft) => {
        draft.ui.upgradeTreeView = {
          scrollLeft: 0,
          scrollTop: 0
        };
  ...
  const upgradesToDraw = UPGRADES.filter((upgrade) => upgrade.visibleWhen(state));
...
function restoreScrollPosition(scrollHost, state) {
  const savedView = state.ui.upgradeTreeView ?? { scrollLeft: 0, scrollTop: 0 };
